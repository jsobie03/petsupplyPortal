	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'You have clocked-in for today';
				}
				else{
					//updates
				...
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'Cannot Clock-out. No clock-in found.';
				}
				else{
					$row = $query->fetch_assoc();
					if($row['time_out'] != '00:00:00'){
						$output['error'] = true;
						$output['message'] = 'You have clocked-out for today';
					}
					else{
						
						$sql = "UPDATE attendance SET time_out = NOW() WHERE id = '".$row['uid']."'";
						if($conn->query($sql)){
							$output['message'] = 'Clocked-out: '.$row['firstname'].' '.$row['lastname'];
